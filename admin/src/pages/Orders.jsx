...
function Orders() {
  ...
  const sortedOrders = sortConfig.key ? sortOrders(orderList, sortConfig.key, sortConfig.direction) : orderList;
        ...
        {orderList.length === 0 ? (
          ...
          <div className="w-full min-w-[59rem]">
            ...
            <div className="grid grid-cols-[auto,repeat(12,minmax(0,1fr))] items-center py-2 gap-2 bg-primary/70 text-white font-medium rounded-md">
              <p className='px-2'>#</p>
              <p className="col-span-3 cursor-pointer" onClick={() => handleSort('orderId')}>
                Order Id
                <img src={assets.dropdown_icon} className='w-2 aspect-square invert-[1]' />
              </p>
              <p>Image</p>
              <p className="col-span-3">Product Name</p>
              <p className="col-span-2 flex items-center gap-1 cursor-pointer" onClick={() => handleSort('date')}>
                Date
                <img src={assets.dropdown_icon} className='w-2 aspect-square invert-[1]' />
              </p>
              <p className="flex items-center gap-1 cursor-pointer" onClick={() => handleSort('amount')}>
                Amount
                <img src={assets.dropdown_icon} className='w-2 aspect-square invert-[1]' />
              </p>
            ...
            {sortedOrders.map((order, index) => (
              ...
              >
                ...
                <p className="col-span-2">{new Date(order.date).toLocaleDateString()}</p>
                <p>{order.amount}</p>
                <select
                  name="status"
                  id="status"
