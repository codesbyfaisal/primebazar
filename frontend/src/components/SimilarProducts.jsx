      ...
      <div className='grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 my-4'>
        {
          similarProducts.map((product, index) => {
            return <ProductCard key={index} id={product._id} images={product.images} name={product.name} price={product.price} sale={product.sale ? product.saleOff : product.sale} />
          })
        }
      </div>
